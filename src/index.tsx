...
function classToComponent(children: ReactNode, className: string, element: HTMLElement): ReactNode {
    ...
    switch (className.toLowerCase()) {
        case 'tw-select-group':
            return <InputWrapper<'select'> children={children} component={SelectInput} container={element as HTMLSelectElement} key={key} />;
        case 'tw-option':
            const { className, textContent, style, value } = element as HTMLOptionElement;
            return <option children={textContent} className={className} css={style.cssText} key={key} value={value} />;
