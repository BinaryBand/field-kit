...
function Signature(
  ...
  const isSigned: boolean = React.useMemo(() => signaturePoints.length > 0, [signaturePoints]);

  const svg = React.useMemo((): string => {
    const fmt = (n: number): string => n.toFixed(0);
    const lines: string[] = [];
    for (const stroke of signaturePoints) {
      const filteredStroke = filterClosePoints(stroke);
  ...
  return (
    <Fragment>
      <SignatureContainer data-readonly={readOnly || undefined}>
        <SignatureInput
          // {...props}
          // placeholder={isSigned || placeholder === 'undefined' ? '' : placeholder}
          readOnly
